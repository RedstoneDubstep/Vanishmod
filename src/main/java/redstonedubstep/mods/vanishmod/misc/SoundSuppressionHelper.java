...
public class SoundSuppressionHelper {
	...
	public static void updateVanishedPlayerMap(ServerPlayer player, boolean vanished) {
		...
		new HashSet<>(vanishedPlayersAndHitResults.keySet()).stream().filter(loopUuid -> player.level().getServer().getPlayerList().getPlayer(loopUuid) == null).forEach(vanishedPlayersAndHitResults::remove);
	}
	...
	public static Player getVanishedPlayerAt(Level level, Vec3 pos, ServerPlayer forPlayer) {
		PlayerList list = forPlayer.level().getServer().getPlayerList();
